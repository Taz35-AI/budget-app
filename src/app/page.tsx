  ...
  return (
    <>
      ...
      <div style={{ background: '#0A1A1A', color: 'rgba(255,255,255,0.80)', ...sans }}>
        ...
        {/* ── HERO ── */}
        <section style={{ padding:'90px 40px 80px', textAlign:'center', maxWidth:860, margin:'0 auto' }}>
          ...
          <h1 style={{ ...serif, fontSize:'clamp(42px, 6vw, 68px)', fontWeight:400, color:'#fff', lineHeight:1.08, letterSpacing:-1, marginBottom:12 }}>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto 0', padding:'0 40px' }}>
          <div style={{ background:'linear-gradient(135deg, rgba(59,122,120,0.14) 0%, rgba(59,122,120,0.06) 100%)', border:'1px solid rgba(59,122,120,0.3)', borderRadius:18, padding:'32px 40px', display:'flex', alignItems:'center', gap:32, flexWrap:'wrap' }}>
            <div style={{ flexShrink:0, width:64, height:64, background:'rgba(59,122,120,0.12)', border:'1px solid rgba(59,122,120,0.3)', borderRadius:18, display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            <div style={{ flex:1, minWidth:260 }}>
              <h2 style={{ ...serif, fontSize:26, color:'#fff', fontWeight:400, marginBottom:6 }}>
              ...
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:14 }}>
                {['✓ No bank sync required','✓ No OAuth tokens stored','✓ No open banking API','✓ Data encrypted at rest','✓ Zero credential exposure'].map(p => (
        ...
        {/* ── TRUST BAR ── */}
        <div style={{ borderTop:'1px solid rgba(255,255,255,0.07)', borderBottom:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.04)', padding:'14px 40px', marginTop:40 }}>
        ...
        <section style={{ padding:'80px 40px', maxWidth:1100, margin:'0 auto' }}>
          ...
          <h2 style={{ ...serif, fontSize:'clamp(30px, 4vw, 44px)', color:'#fff', fontWeight:400, lineHeight:1.15, marginBottom:48 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
            ...
            <div style={{ borderRadius:16, overflow:'hidden' }}>
              ...
              <div style={{ border:'1px solid rgba(229,83,75,0.15)', borderTop:'none', borderRadius:'0 0 16px 16px', overflow:'hidden' }}>
                {[
                  ['Demand your banking password', 'or open-banking login to sync transactions automatically.'],
                  ['Miscategorise 20–30% of transactions', ', forcing you to spend more time fixing errors than budgeting.'],
                  ['Break bank connections for months', ', leaving your data stale and your budget meaningless.'],
                  ['Charge $15–$18/month', ' with aggressive upsell prompts and locked-down free tiers.'],
                  ['No balance forecast', ' — they tell you what you\'ve spent, never where you\'re heading.'],
                ].map(([bold, rest]) => (
            ...
            <div style={{ borderRadius:16, overflow:'hidden' }}>
              ...
              <div style={{ border:'1px solid rgba(59,122,120,0.3)', borderTop:'none', borderRadius:'0 0 16px 16px', overflow:'hidden' }}>
                {[
                  ...
                  ['Always accurate.', ' Your data is what you entered — no broken APIs, no duplicate imports.'],
        ...
        {/* ── DASHBOARD PREVIEW ── */}
        <section style={{ padding:'0 40px 80px', maxWidth:1180, margin:'0 auto' }}>
          ...
          <h2 style={{ ...serif, fontSize:'clamp(28px, 3.5vw, 40px)', color:'#fff', fontWeight:400, lineHeight:1.15, marginBottom:28 }}>
            Your finances, at a glance.
          </h2>
          {/* Browser chrome */}
          <div style={{ background:'#0D1F1E', border:'1px solid rgba(255,255,255,0.09)', borderRadius:18, overflow:'hidden', boxShadow:'0 24px 80px rgba(0,0,0,0.5)' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:6, padding:'10px 12px 9px', borderBottom:'1px solid rgba(255,255,255,0.07)' }}>
                      <div style={{ display:'flex', gap:4, flex:1, minWidth:0 }}>
                        {(['Combined','Current','Savings'] as string[]).map((tab, i) => (
                          <span key={tab} style={{ fontSize:9, fontWeight:700, padding:'3px 8px', borderRadius:6, border:'1px solid', flexShrink:0,
                            background: i===0 ? '#3B7A78' : 'rgba(255,255,255,0.05)',
                            color: i===0 ? '#fff' : 'rgba(255,255,255,0.35)',
                            borderColor: i===0 ? '#3B7A78' : 'rgba(255,255,255,0.10)',
                          }}>{tab}</span>
                        ))}
                      </div>
                      <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, color:'#fff', whiteSpace:'nowrap' }}>April 2025</span>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:2, padding:'2px 8px 8px', flex:1 }}>
                      {(([
                        ...
                        <div key={i} style={{
                          ...
                          minHeight:38,
                          border:'1px solid',
                          background: cell.grey ? 'rgba(255,255,255,0.02)' :
                            cell.sel ? 'rgba(59,122,120,0.12)' :
                            cell.pos ? 'rgba(59,122,120,0.07)' : 'rgba(255,255,255,0.04)',
                          borderColor: cell.grey ? 'rgba(255,255,255,0.04)' :
                            cell.sel ? 'rgba(59,122,120,0.50)' :
                            cell.today ? 'rgba(59,122,120,0.30)' : 'rgba(255,255,255,0.06)',
                        }}>
                          {/* Day number */}
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                            <span style={{
                              ...
                              background: cell.today ? '#3B7A78' : 'transparent',
                              color: cell.grey ? 'rgba(255,255,255,0.15)' : cell.today ? '#fff' : cell.sel ? '#4E9E9B' : 'rgba(255,255,255,0.30)',
                            }}>{cell.d}</span>
                          ...
                          {/* Balance */}
                          {cell.bal && (
                            <span style={{ marginTop:'auto', fontSize:7, fontWeight:800, color: cell.pos ? '#4E9E9B' : '#E5534B', lineHeight:1 }}>{cell.bal}</span>
                          )}
                          {/* Bottom accent */}
                          {cell.bal && !cell.sel && (
                            <div style={{ position:'absolute', bottom:0, left:0, right:0, height:2, borderRadius:'0 0 6px 6px', background: cell.pos ? 'linear-gradient(90deg,rgba(59,122,120,0.5),transparent)' : 'linear-gradient(90deg,rgba(229,83,75,0.5),transparent)' }} />
                          )}
                        </div>
                      ))}
                    </div>
        ...
        <section style={{ padding:'0 40px 80px', maxWidth:1100, margin:'0 auto' }}>
          ...
          <h2 style={{ ...serif, fontSize:'clamp(30px, 4vw, 44px)', color:'#fff', fontWeight:400, lineHeight:1.15, marginBottom:40 }}>Everything you need. Nothing you don&apos;t.</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:16 }}>
            {/* Wide card */}
            <div style={{ background:'#0F2222', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:24, gridColumn:'span 2' }}>
            ...
            {[
              ...
              { icon:<path strokeLinecap="round" strokeLinejoin="round" d="M7 16V4m0 0L3 8m4-4l4 4M17 8v12m0 0l4-4m-4 4l-4-4"/>, title:'Transfer between accounts', body:'Move money between accounts in one tap. Expense on the source, income on the destination — balances always accurate.' },
            ...
            {/* Mobile wide card */}
            <div style={{ background:'rgba(59,122,120,0.10)', border:'1px solid rgba(59,122,120,0.3)', borderRadius:16, padding:24, gridColumn:'span 2' }}>
        ...
        {/* ── HOW IT WORKS ── */}
        <section style={{ padding:'80px 40px', maxWidth:900, margin:'0 auto', textAlign:'center', borderTop:'1px solid rgba(255,255,255,0.07)' }}>
          ...
          <h2 style={{ ...serif, fontSize:'clamp(30px, 4vw, 44px)', color:'#fff', fontWeight:400, marginBottom:12 }}>Up and running in 3 minutes</h2>
          ...
          <div className="steps-line" style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:32, position:'relative' }}>
            {[
              { n:'1', title:'Create your account', body:'Sign up free with email or Google. No credit card, no bank login. Takes 30 seconds.' },
              { n:'2', title:'Add accounts & transactions', body:'Set up accounts, then add recurring income and expenses. Salary, rent, subscriptions — enter once, tracked forever.' },
              { n:'3', title:'See your financial future', body:'Instantly see your projected balance day by day, weeks, months and years ahead. No guessing, no surprises.' },
            ].map(s => (
              <div key={s.n} style={{ position:'relative' }}>
                <div style={{ width:56, height:56, borderRadius:'50%', background:'rgba(59,122,120,0.12)', border:'1px solid rgba(59,122,120,0.3)', display:'flex', alignItems:'center', justifyContent:'center', ...serif, fontSize:22, color:'#4E9E9B', margin:'0 auto 18px', position:'relative', zIndex:1 }}>{s.n}</div>
        ...
        {/* ── FAQ ── */}
        <section style={{ padding:'80px 40px', maxWidth:780, margin:'0 auto' }}>
          ...
          <h2 style={{ ...serif, fontSize:'clamp(30px, 4vw, 44px)', color:'#fff', fontWeight:400, marginBottom:32, textAlign:'center' }}>Still not sure? We&apos;ve heard it all.</h2>
          {[
        ...
        {/* ── CTA ── */}
        <section style={{ padding:'80px 40px', textAlign:'center', borderTop:'1px solid rgba(255,255,255,0.07)' }}>
          <div style={{ maxWidth:600, margin:'0 auto' }}>
            <h2 style={{ ...serif, fontSize:'clamp(34px, 5vw, 52px)', color:'#fff', fontWeight:400, marginBottom:14, lineHeight:1.1 }}>
            ...
            <Link href="/signup" style={{ background:'#3B7A78', color:'#fff', fontSize:16, fontWeight:600, padding:'15px 34px', borderRadius:12, textDecoration:'none', display:'inline-flex', alignItems:'center', gap:8 }}>
              Create free account
        ...
        {/* ── FOOTER ── */}
        <footer style={{ borderTop:'1px solid rgba(255,255,255,0.07)', padding:'28px 40px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
